...
export default class Keyboard {
  constructor(targetVCOs, targetVCA, targetDOM = window.document) {
    this.baseFrequency = 261.63;
    this.targetDOMNode = targetDOM;
    this.targetVCA = targetVCA;
    this.targetVCOs = targetVCOs;
  ...
  keypress(e) {
    ...
    if (keyPosition >= 0) {
      this.targetVCOs.forEach(vco => {
        vco.frequency = this.baseFrequency * Math.pow(2, keyPosition / 12);
      });
